...
const STATIC_STORE_KEY = "engineering-cell-admin-static-store-v1";
let memoryStaticStore = null;
...
function nowDateTimeStringLocal(includeSeconds = false) {
  const now = new Date();
  const base = `${now.getFullYear()}-${padDatePart(now.getMonth() + 1)}-${padDatePart(now.getDate())} ${padDatePart(now.getHours())}:${padDatePart(now.getMinutes())}`;
  return includeSeconds ? `${base}:${padDatePart(now.getSeconds())}` : base;
}
...
function extractSeedGeneProject(project) {
  return {
    id: project.id,
    name: project.name,
    strain: project.strain,
    ...
    dataset: {
      ...
      size: project.dataset?.size || "0 MB",
      sites: project.dataset?.sites || "0",
      uploadedAt: project.dataset?.uploadedAt || nowDateTimeStringLocal(true)
    }
  };
}

function buildSeedAnalysisItem(moduleKey, row, index) {
  const status = row.status?.text
    ? row.status
    : {
        text: row.statusText || "进行中",
        className: row.statusClass || statusTextToClass(row.statusText || "进行中")
      };

  if (moduleKey === "omics") {
    return hydrateOmicsItem({
      id: row.id || `omics-${index + 1}`,
      name: row.name,
      strain: row.strain,
      source: row.source,
      ...
      createdAt: row.createdAt || nowDateTimeStringLocal(),
      status,
      actions: buildAnalysisActions(moduleKey)
    });
  }

  if (moduleKey === "process") {
    return hydrateProcessItem({
      id: row.id || `process-${index + 1}`,
      name: row.name,
      model: row.model,
      strain: row.strain,
      goal: row.goal,
      ...
      currentYield: row.currentYield,
      optimizedYield: row.optimizedYield,
      confidenceRange: row.confidenceRange,
      createdAt: row.createdAt || nowDateTimeStringLocal(),
      status,
      actions: buildAnalysisActions(moduleKey)
    });
  ...
  if (moduleKey === "full") {
    return hydrateFullItem({
      id: row.id || `full-${index + 1}`,
      name: row.name,
      strain: row.strain,
      description: row.description,
      modules: row.modules,
      targetProduct: row.targetProduct,
      patentRisk: row.patentRisk,
      createdAt: row.createdAt || todayStringLocal(),
      status,
      actions: buildAnalysisActions(moduleKey)
    });
  }

  const matchedServiceEntry = Object.entries(serviceCellLibrary).find(([, item]) => item.basicInfo?.code === row.code);
  const serviceId = row.id || matchedServiceEntry?.[0] || `service-${index + 1}`;
  const servicePreset = matchedServiceEntry?.[1] || {};
  const serviceStatusText = status.text || servicePreset.basicInfo?.statusText || "已验证";

  return hydrateServiceItem({
    id: serviceId,
    code: row.code || servicePreset.basicInfo?.code || "",
    name: row.name || servicePreset.basicInfo?.name || "",
    strain: row.strain || servicePreset.basicInfo?.strain || "谷氨酸棒杆菌",
    gene: row.gene || servicePreset.basicInfo?.gene || "",
    description: row.description || servicePreset.basicInfo?.description || "",
    basicInfo: {
      ...(servicePreset.basicInfo || {}),
      code: row.code || servicePreset.basicInfo?.code || "",
      name: row.name || servicePreset.basicInfo?.name || "",
      strain: row.strain || servicePreset.basicInfo?.strain || "谷氨酸棒杆菌",
      gene: row.gene || servicePreset.basicInfo?.gene || "",
    ...
    cultureInfo: {
      ...(servicePreset.cultureInfo || {})
    },
    analysisInfo: {
      ...(servicePreset.analysisInfo || {})
    },
    applicationInfo: {
      ...(servicePreset.applicationInfo || {})
    },
    createdAt: row.createdAt || nowDateTimeStringLocal(),
    status: {
      text: serviceStatusText,
      className: status.className || statusTextToClass(serviceStatusText)
    },
    actions: buildAnalysisActions(moduleKey)
  });
}

function flattenSensorRecords(moduleKey) {
  return sensorModules[moduleKey].batches.flatMap((batch) =>
    batch.records.map((record) => ({
      ...
      metrics: deepClone(record.metrics || []),
      detail: deepClone(record.detail || []),
      detailCards: deepClone(record.detailCards || [])
    }))
  );
}

function buildInitialStaticStore() {
  return {
    version: 1,
    geneProjects: Object.values(geneProjectLibrary).map(extractSeedGeneProject),
    analysisItems: {
      omics: analysisPages.omics.rows.map((row, index) => buildSeedAnalysisItem("omics", row, index)),
      process: analysisPages.process.rows.map((row, index) => buildSeedAnalysisItem("process", row, index)),
      full: analysisPages.full.rows.map((row, index) => buildSeedAnalysisItem("full", row, index)),
      service: analysisPages.service.rows.map((row, index) => buildSeedAnalysisItem("service", row, index))
    },
    sensorRecords: {
      physical: flattenSensorRecords("physical"),
      biological: flattenSensorRecords("biological")
    },
    sensorThresholds: {
      physical: deepClone(sensorModules.physical.thresholdRows),
      biological: deepClone(sensorModules.biological.thresholdRows)
    }
  };
}

const STATIC_STORE_DEFAULTS = buildInitialStaticStore();

function mergeStaticStoreShape(store) {
  return {
    version: STATIC_STORE_DEFAULTS.version,
    geneProjects: Array.isArray(store?.geneProjects) ? store.geneProjects : deepClone(STATIC_STORE_DEFAULTS.geneProjects),
    analysisItems: {
      omics: Array.isArray(store?.analysisItems?.omics) ? store.analysisItems.omics : deepClone(STATIC_STORE_DEFAULTS.analysisItems.omics),
      process: Array.isArray(store?.analysisItems?.process) ? store.analysisItems.process : deepClone(STATIC_STORE_DEFAULTS.analysisItems.process),
      full: Array.isArray(store?.analysisItems?.full) ? store.analysisItems.full : deepClone(STATIC_STORE_DEFAULTS.analysisItems.full),
      service: Array.isArray(store?.analysisItems?.service) ? store.analysisItems.service : deepClone(STATIC_STORE_DEFAULTS.analysisItems.service)
    },
    sensorRecords: {
      physical: Array.isArray(store?.sensorRecords?.physical) ? store.sensorRecords.physical : deepClone(STATIC_STORE_DEFAULTS.sensorRecords.physical),
      biological: Array.isArray(store?.sensorRecords?.biological) ? store.sensorRecords.biological : deepClone(STATIC_STORE_DEFAULTS.sensorRecords.biological)
    },
    sensorThresholds: {
      physical: Array.isArray(store?.sensorThresholds?.physical) ? store.sensorThresholds.physical : deepClone(STATIC_STORE_DEFAULTS.sensorThresholds.physical),
      biological: Array.isArray(store?.sensorThresholds?.biological) ? store.sensorThresholds.biological : deepClone(STATIC_STORE_DEFAULTS.sensorThresholds.biological)
    }
  };
}

function readStaticStore() {
  if (memoryStaticStore) {
    return deepClone(memoryStaticStore);
  }

  try {
    const raw = window.localStorage.getItem(STATIC_STORE_KEY);
    if (!raw) {
      memoryStaticStore = deepClone(STATIC_STORE_DEFAULTS);
      return deepClone(memoryStaticStore);
    }
    memoryStaticStore = mergeStaticStoreShape(JSON.parse(raw));
    return deepClone(memoryStaticStore);
  } catch {
    memoryStaticStore = deepClone(STATIC_STORE_DEFAULTS);
    return deepClone(memoryStaticStore);
...
function writeStaticStore(store) {
  const normalized = mergeStaticStoreShape(store);
  memoryStaticStore = deepClone(normalized);
  try {
    window.localStorage.setItem(STATIC_STORE_KEY, JSON.stringify(normalized));
  } catch {}
  return deepClone(normalized);
}

function applyPersistedSensorThresholds() {
  const store = readStaticStore();
  ["physical", "biological"].forEach((moduleKey) => {
    sensorModules[moduleKey].thresholdRows = deepClone(
      store.sensorThresholds[moduleKey] || STATIC_STORE_DEFAULTS.sensorThresholds[moduleKey]
    );
  });
}
...
function parseRequestBody(options = {}) {
  ...
  if (typeof options.body === "string") {
    try {
      ...
      throw createClientError("请求体不是合法 JSON");
    }
  }

  if (typeof options.body === "object") {
    return deepClone(options.body);
  }

  return {};
}

function validateGeneProjectInput(input, currentProject = null) {
  const now = nowDateTimeStringLocal();
  const datasetNow = nowDateTimeStringLocal(true);
  const datasetInput = input.dataset || {};
  const baseDataset = currentProject?.dataset || {};
  const project = {
    id: currentProject?.id || String(input.id || "").trim() || generateLocalId("g"),
    ...
    dataset: {
      name:
        String(
          datasetInput.name ??
            input.datasetName ??
            baseDataset.name ??
            `${String(input.name || currentProject?.name || "未命名项目").trim()}_数据集`
        ).trim() || `${String(input.name || currentProject?.name || "未命名项目").trim()}_数据集`,
      ...
      uploadedAt:
        String(datasetInput.uploadedAt ?? input.datasetUploadedAt ?? baseDataset.uploadedAt ?? datasetNow).trim() ||
        datasetNow
    }
  };

  if (!project.name) {
    throw createClientError("项目名称不能为空");
  }
  if (!project.strain) {
    throw createClientError("菌株类型不能为空");
  }
  if (!project.phenotype) {
    throw createClientError("表型类型不能为空");
  }
  if (!project.method) {
    throw createClientError("分析方法不能为空");
  }
  if (!project.threshold) {
    throw createClientError("显著性阈值不能为空");
  }
  if (!project.description) {
    throw createClientError("项目描述不能为空");
  }

  return project;
}

function validateAnalysisItemInput(input, current = null) {
  const moduleKey = String(input.module || current?.module || "").trim();
  if (!persistedAnalysisModules.includes(moduleKey)) {
    throw createClientError("模块类型不支持");
  }
  ...
  const createdAt = String(input.createdAt || currentItem.createdAt || nowDateTimeStringLocal()).trim() || nowDateTimeStringLocal();
  const item = {
    id: currentItem.id || String(input.id || "").trim() || generateLocalId(moduleKey),
    ...currentItem,
    ...deepClone(input),
    createdAt,
    status: {
      text: statusText || "进行中",
      className: statusClass || statusTextToClass(statusText || "进行中")
    }
  };
  ...
  if (!String(item.name || "").trim()) {
    throw createClientError("名称不能为空");
  }
  if (moduleKey === "service" && !String(item.code || "").trim()) {
    throw createClientError("细胞编号不能为空");
  }
...
function validateSensorRecordInput(input) {
  const module = String(input.module || "").trim();
  if (!["physical", "biological"].includes(module)) {
    throw createClientError("传感器模块不支持");
  }

  const prefix = module === "physical" ? "p" : "b";
  const record = {
    id: String(input.id || "").trim() || generateLocalId(prefix),
    module,
    batchId: String(input.batchId || "").trim(),
    time: String(input.time || "").trim() || nowDateTimeStringLocal(true),
    user: String(input.user || "").trim(),
    statusText: String(input.statusText || "").trim() || "正常",
    statusClass: String(input.statusClass || "").trim() || "is-normal",
    metrics: Array.isArray(input.metrics) ? deepClone(input.metrics) : [],
    detail: Array.isArray(input.detail) ? deepClone(input.detail) : [],
    detailCards: Array.isArray(input.detailCards) ? deepClone(input.detailCards) : []
  };

  if (!record.batchId) {
    throw createClientError("批次不能为空");
  }
  if (!record.user) {
    throw createClientError("录入人员不能为空");
  }
  if (!record.metrics.length) {
    throw createClientError("至少需要一项录入指标");
  }

  return record;
}

function saveGeneProjectToStore(project) {
  const store = readStaticStore();
  const index = store.geneProjects.findIndex((item) => item.id === project.id);
  if (index >= 0) {
    store.geneProjects[index] = deepClone(project);
  } else {
    store.geneProjects.unshift(deepClone(project));
  }
  writeStaticStore(store);
  return deepClone(project);
}

function deleteGeneProjectFromStore(projectId) {
  const store = readStaticStore();
  store.geneProjects = store.geneProjects.filter((item) => item.id !== projectId);
  writeStaticStore(store);
}

function getStoredAnalysisEntry(moduleKey, itemId) {
  const store = readStaticStore();
  const item = (store.analysisItems[moduleKey] || []).find((entry) => entry.id === itemId);
  return item ? { module: moduleKey, item: deepClone(item) } : null;
}

function saveAnalysisEntryToStore(entry) {
  const store = readStaticStore();
  const index = store.analysisItems[entry.module].findIndex((item) => item.id === entry.item.id);
  if (index >= 0) {
    store.analysisItems[entry.module][index] = deepClone(entry.item);
  } else {
    store.analysisItems[entry.module].unshift(deepClone(entry.item));
  }
  writeStaticStore(store);
  return { module: entry.module, item: deepClone(entry.item) };
}

function deleteAnalysisEntryFromStore(itemId) {
  const store = readStaticStore();
  persistedAnalysisModules.forEach((moduleKey) => {
    store.analysisItems[moduleKey] = store.analysisItems[moduleKey].filter((item) => item.id !== itemId);
  });
  writeStaticStore(store);
}

function saveSensorRecordToStore(record) {
  const store = readStaticStore();
  const list = store.sensorRecords[record.module];
  const index = list.findIndex((item) => item.id === record.id);
  if (index >= 0) {
    list[index] = deepClone(record);
  } else {
    list.unshift(deepClone(record));
  }
  writeStaticStore(store);
  return deepClone(record);
}

function deleteSensorRecordFromStore(recordId) {
  const store = readStaticStore();
  ["physical", "biological"].forEach((moduleKey) => {
    store.sensorRecords[moduleKey] = store.sensorRecords[moduleKey].filter((item) => item.id !== recordId);
  });
  writeStaticStore(store);
}

function saveSensorThresholdsToStore(moduleKey, config) {
  const store = readStaticStore();
  store.sensorThresholds[moduleKey] = deepClone(config);
  writeStaticStore(store);
  return {
    module: moduleKey,
    config: deepClone(store.sensorThresholds[moduleKey])
  };
}

function resetSensorThresholdsToDefault(moduleKey) {
  const defaultRows = deepClone(STATIC_STORE_DEFAULTS.sensorThresholds[moduleKey] || []);
  sensorModules[moduleKey].thresholdRows = deepClone(defaultRows);
  saveSensorThresholdsToStore(moduleKey, defaultRows);
  return defaultRows;
}
...
function buildImportedServiceInput(mappedRow, current) {
  ...
  return {
    ...
    basicInfo: {
      ...
      strain:
        mappedRow.strain ||
        mappedRow.basicInfo?.strain ||
        currentItem.basicInfo?.strain ||
        currentItem.strain ||
        "谷氨酸棒杆菌",
      ...
      description:
        ...
        mappedRow.basicInfo?.description ||
        currentItem.basicInfo?.description ||
...
function importServiceWorkbookClient(input) {
  ...
  if (!fileName || !fileContent) {
    throw createClientError("缺少导入文件");
  }
  if (![".csv", ".xlsx", ".xls"].includes(extension)) {
    throw createClientError("仅支持导入 .csv、.xlsx、.xls 文件");
  }
  if (fileSize > 50 * 1024 * 1024) {
    throw createClientError("文件大小不能超过 50MB");
  }
  if (!window.XLSX) {
    throw createClientError("缺少表格解析能力");
  }

  const bytes = decodeBase64ToUint8Array(fileContent);
  const workbook = window.XLSX.read(bytes, { type: "array", codepage: 65001 });
  const firstSheetName = workbook.SheetNames[0];
  if (!firstSheetName) {
    throw createClientError("未读取到工作表内容");
  }
  ...
  if (headerIndex === -1) {
    throw createClientError("导入文件没有可用数据");
  }
  ...
  rows.forEach((row, index) => {
    try {
      ...
      if (!String(mapped.name || "").trim()) {
        throw createClientError("细胞名称不能为空");
      }
      ...
      const validated = validateAnalysisItemInput(
        {
          module: "service",
          id: current?.item?.id,
          createdAt: current?.item?.createdAt || nowDateTimeStringLocal(),
          ...buildImportedServiceInput(mapped, current)
        },
        current
      );
      const saved = saveAnalysisEntryToStore(validated);
...
async function handleStaticApiRequest(url, options = {}) {
  ...
  if (requestUrl.pathname === "/api/gene-projects" && method === "GET") {
    const store = readStaticStore();
    return { items: deepClone(store.geneProjects) };
  }

  if (requestUrl.pathname === "/api/gene-projects" && method === "POST") {
    return {
      item: saveGeneProjectToStore(validateGeneProjectInput(body))
    };
  }

  const geneMatch = requestUrl.pathname.match(/^\/api\/gene-projects\/([^/]+)$/);
  if (geneMatch) {
    const projectId = geneMatch[1];
    const store = readStaticStore();
    const current = store.geneProjects.find((item) => item.id === projectId);
    if (!current) {
      throw createClientError("分析项目不存在");
    }
    if (method === "GET") {
      return { item: deepClone(current) };
    }
    if (method === "PUT") {
      return {
        item: saveGeneProjectToStore(validateGeneProjectInput(body, current))
      };
    }
    if (method === "DELETE") {
      deleteGeneProjectFromStore(projectId);
      return { success: true };
    }
  }

  if (requestUrl.pathname === "/api/analysis-items" && method === "GET") {
    const moduleKey = String(requestUrl.searchParams.get("module") || "").trim();
    if (!moduleKey) {
      throw createClientError("缺少模块参数");
    }
    const store = readStaticStore();
    return { items: deepClone(store.analysisItems[moduleKey] || []) };
  }

  if (requestUrl.pathname === "/api/analysis-items" && method === "POST") {
    return saveAnalysisEntryToStore(validateAnalysisItemInput(body));
  }

  const analysisMatch = requestUrl.pathname.match(/^\/api\/analysis-items\/([^/]+)$/);
  if (analysisMatch) {
    const itemId = analysisMatch[1];
    const current =
      persistedAnalysisModules.map((moduleKey) => getStoredAnalysisEntry(moduleKey, itemId)).find(Boolean) || null;
    if (!current) {
      throw createClientError("记录不存在");
    }
    if (method === "GET") {
      return deepClone(current);
    }
    if (method === "PUT") {
      return saveAnalysisEntryToStore(validateAnalysisItemInput({ ...body, module: current.module }, current));
    }
    if (method === "DELETE") {
      deleteAnalysisEntryFromStore(itemId);
      return { success: true };
    }
  }

  if (requestUrl.pathname === "/api/analysis-import/service" && method === "POST") {
    return importServiceWorkbookClient(body);
  }
  ...
  if (requestUrl.pathname === "/api/sensor-records" && method === "GET") {
    ...
    const store = readStaticStore();
    return { items: deepClone(store.sensorRecords[moduleKey] || []) };
  }

  if (requestUrl.pathname === "/api/sensor-records" && method === "POST") {
    return {
      item: saveSensorRecordToStore(validateSensorRecordInput(body))
    };
  }

  const sensorMatch = requestUrl.pathname.match(/^\/api\/sensor-records\/([^/]+)$/);
  if (sensorMatch && method === "DELETE") {
    deleteSensorRecordFromStore(sensorMatch[1]);
    return { success: true };
  }

  if (requestUrl.pathname === "/api/sensor-thresholds" && method === "GET") {
    const moduleKey = String(requestUrl.searchParams.get("module") || "").trim();
    if (!moduleKey) {
      throw createClientError("缺少模块参数");
    }
  ...
  if (requestUrl.pathname === "/api/sensor-thresholds" && method === "POST") {
    const moduleKey = String(body.module || "").trim();
    if (!moduleKey) {
      throw createClientError("模块参数不能为空");
    }
    if (!Array.isArray(body.config)) {
      throw createClientError("配置数据格式错误");
    }
    return saveSensorThresholdsToStore(moduleKey, body.config);
  }

  throw createClientError("接口不存在");
}
...
function collectThresholdRowsFromModal(moduleKey) {
  const currentRows = sensorModules[moduleKey].thresholdRows || [];
  const domRows = [...document.querySelectorAll(".config-table tbody tr")];

  return currentRows.map((row, index) => {
    const inputs = [...(domRows[index]?.querySelectorAll("input") || [])];
    return {
      ...row,
      min: inputs[0]?.value.trim() ?? row.min,
      max: inputs[1]?.value.trim() ?? row.max,
      alert: inputs[2]?.value.trim() ?? row.alert
    };
  });
}
...
document.addEventListener("click", async (event) => {
  ...
  const staticModalAction = event.target.closest("[data-modal-action]");
  if (staticModalAction) {
    const [action, moduleKey] = staticModalAction.dataset.modalAction.split("|");

    if (action === "save-threshold") {
      try {
        const payload = await apiRequest("/api/sensor-thresholds", {
          method: "POST",
          body: JSON.stringify({
            module: moduleKey,
            config: collectThresholdRowsFromModal(moduleKey)
          })
        });
        sensorModules[moduleKey].thresholdRows = deepClone(payload.config || []);
        state.modal = null;
        renderApp();
        showToast("阈值配置已保存");
      } catch (error) {
        showToast(error.message || "阈值保存失败");
      }
      return;
    }

    if (action === "reset-threshold") {
      sensorModules[moduleKey].thresholdRows = resetSensorThresholdsToDefault(moduleKey);
      renderApp();
      showToast("已恢复默认阈值");
      return;
    }
    ...
    if (action === "confirm-import") {
      state.modal = null;
      renderApp();
      showToast(moduleKey === "physical" ? "物理参数数据已导入" : "代谢物数据已导入");
      return;
    }
  }
