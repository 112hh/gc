...
function validateTheme(input, existingId) {
  ...
  if (!theme.name) {
    throw badRequest("主题名称不能为空");
  }
  if (!theme.category) {
    throw badRequest("分类不能为空");
  }
  if (!["high", "medium", "low"].includes(theme.priority)) {
    throw badRequest("优先级必须是 high、medium 或 low");
  }
  if (!theme.owner) {
    throw badRequest("负责人不能为空");
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(theme.updated)) {
    throw badRequest("更新时间必须是 YYYY-MM-DD");
  }
  if (!theme.summary) {
    throw badRequest("摘要不能为空");
  }
  if (!theme.objective) {
    throw badRequest("目标说明不能为空");
  }
  if (theme.metrics.length === 0) {
    throw badRequest("指标至少填写一项，格式为 label:value");
  }
  if (theme.milestones.length === 0) {
    throw badRequest("里程碑至少填写一项");
  }
...
function importServiceWorkbook(input) {
  ...
  if (!fileName || !fileContent) {
    throw badRequest("缺少导入文件");
  }
  if (![".csv", ".xlsx", ".xls"].includes(extension)) {
    throw badRequest("仅支持导入 .csv、.xlsx、.xls 文件");
  }
  if (fileSize > 50 * 1024 * 1024) {
    throw badRequest("文件大小不能超过 50MB");
  }
  ...
  rows.forEach((row, index) => {
    try {
      ...
      if (!String(mapped.name || "").trim()) {
        throw badRequest("细胞名称不能为空");
      }
      if (!String(mapped.strain || "").trim()) {
        throw badRequest("菌株类型不能为空");
      }
...
function validateSensorRecord(input) {
  ...
  if (!record.batchId) {
    throw badRequest("批次不能为空");
  }
  if (!record.user) {
    throw badRequest("录入人员不能为空");
  }
  if (!record.metrics.length) {
    throw badRequest("至少需要一项录入指标");
  }
...
function validateGeneProject(input, currentProject = null) {
  ...
  if (!project.phenotype) {
    throw badRequest("表型类型不能为空");
  }
  if (!project.method) {
    throw badRequest("分析方法不能为空");
  }
  if (!project.threshold) {
    throw badRequest("显著性阈值不能为空");
  }
  if (!project.description) {
    throw badRequest("项目描述不能为空");
  }
...
async function handler(request, response) {
  try {
    ...
    sendJson(response, statusCode, { message: error.message || "服务器内部错误" });
  }
}
